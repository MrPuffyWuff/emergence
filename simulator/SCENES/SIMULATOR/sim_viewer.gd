...
# Called every frame. 'adelta' is the elapsed time since the previous frame.
func _process(delta: float) -> void:
	var horiz_dir : Vector2 = Input.get_vector("camera_left_strafe", "camera_right_strafe", "camera_foward", "camera_backward")
	horiz_dir = horiz_dir.rotated(-$CameraPivot.rotation.y)
	print(horiz_dir)
	$CameraPivot.velocity.x = horiz_dir.x * GlobalsVars.game_speed
	$CameraPivot.velocity.z = horiz_dir.y * GlobalsVars.game_speed
	var vertical_dir : float = Input.get_axis("camera_descend","camera_ascend")
	$CameraPivot.velocity.y = vertical_dir * GlobalsVars.game_speed
	$CameraPivot.move_and_slide()
